...
const LaporanDistribusiKunjunganKelasPanel = () => {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      ...
      try {
        const res = await axios.get(`${API_BASE_URL}/laporan/kunjungan-distribusi-kelas`, {
          params: {
            tahun,
            bulan,
          },
        ...
        console.error(err);
        setError('Laporan distribusi kunjungan per kelas gagal dimuat.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <section className="bg-white rounded-xl shadow p-6 border border-gray-100 text-[#1A1A1A]">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-8">
        <div>
          <h1 className="text-2xl font-bold font-montserrat">Laporan Distribusi Kunjungan Berdasarkan Kelas</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <div className="rounded-xl border border-emerald-100 bg-emerald-50 p-5">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#2E7D32]">Kunjungan Valid</p>
          <p className="mt-3 text-4xl font-bold font-montserrat text-[#1A1A1A]">
            {(report.summary.total_kunjungan_valid || 0).toLocaleString('id-ID')}
          </p>
        </div>
        <div className="rounded-xl border border-amber-100 bg-amber-50 p-5">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#EDA60F]">Data Tidak Dihitung</p>
          <p className="mt-3 text-4xl font-bold font-montserrat text-[#1A1A1A]">
            {(report.summary.total_kunjungan_tidak_valid || 0).toLocaleString('id-ID')}
          </p>
        </div>
      </div>

      {loading ? (
        <div className="py-16 text-center text-[#585858]">Memuat distribusi kunjungan per kelas...</div>
      ) : (
          ...
          <div>
            <table className="w-full text-left">
              ...
              <tbody>
                {report.data.map((item, index) => (
                  <tr key={`${item.kelas}-${index}`} className="border-b text-sm hover:bg-blue-50/40 transition-colors">
                    <td className="p-4 text-center text-[#7D7D7E]">{index + 1}</td>
                    <td className="p-4 font-semibold">{item.kelas}</td>
            ...
            {report.data.every((item) => item.jumlah_kunjungan === 0) && (
              <div className="mt-8 rounded-xl border border-dashed border-gray-300 bg-gray-50 px-6 py-10 text-center text-[#585858]">
                Belum ada data kunjungan valid pada periode yang dipilih.
              </div>
            )}
          </div>

          <div className="rounded-xl border border-gray-200 bg-gray-50 p-5">
            <h2 className="text-lg font-bold font-montserrat mb-5">Grafik Distribusi Kelas</h2>
            <div className="space-y-4">
              {report.data.map((item) => (
                <div key={item.kelas}>
                  <div className="flex items-center justify-between text-sm mb-2">
                    <span className="font-bold">{item.kelas}</span>
                  ...
                  <div className="h-4 rounded-full bg-white border border-gray-200 overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all ${barColors[item.kelas] || 'bg-[#265F9C]'}`}
            ...
            <div className="mt-6 rounded-xl bg-white border border-gray-200 p-4">
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#585858]">Kelas Aktif</p>
              <p className="mt-2 text-3xl font-bold font-montserrat text-[#1A1A1A]">
                {(report.summary.jumlah_kelas_aktif || 0).toLocaleString('id-ID')}
              </p>
            </div>
          </div>
        </div>
...
export default LaporanDistribusiKunjunganKelasPanel;
